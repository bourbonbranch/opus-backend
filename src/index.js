...
const registerPlannerRoutes = require('./planner-api');
registerPlannerRoutes(app, pool);

// Start server
app.post('/api/migrate-planner-tables', async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    await client.query('COMMIT');
    res.json({ success: true, message: 'Planner tables created successfully' });
  } catch (err) {
    await client.query('ROLLBACK');
    console.error('Migration error:', err);
    res.status(500).json({ error: err.message });
...
app.get('/api/students/rehearsals', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT e.id, e.ensemble_id, e.name as title, e.type, e.start_time, e.end_time, e.description, r.name as location
       FROM events e
       LEFT JOIN rooms r ON e.room_id = r.id
       WHERE e.ensemble_id = $1 AND e.start_time >= NOW()
